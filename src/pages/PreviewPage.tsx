...
export const PreviewPageSimple: React.FC = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      setIsLoading(true);
      console.log('PreviewPage: Loading data, pageId:', pageId);
      console.log('PreviewPage: Current editor state:', {
      ...
      try {
        // 强制从localStorage获取最新数据，确保数据是最新的
        const savedProject = localStorage.getItem('low-code-project');
        console.log('PreviewPage: Saved project exists:', !!savedProject);

        if (savedProject) {
          const project: ProjectConfig = JSON.parse(savedProject);
          console.log('PreviewPage: Parsed project:', {
            id: project.id,
            pagesCount: project.pages.length,
            pageIds: project.pages.map(p => p.id),
          });

          let targetPage: PageConfig | null = null;

          if (pageId) {
            // 根据页面ID查找特定页面
            targetPage = project.pages.find(page => page.id === pageId) || null;
            console.log('PreviewPage: Target page found:', !!targetPage, targetPage?.id);
          } else {
            // 没有指定页面ID，使用第一个页面
            targetPage = project.pages.length > 0 ? project.pages[0] : null;
            console.log('PreviewPage: Using first page:', targetPage?.id);
          }

          if (targetPage) {
            console.log('PreviewPage: Setting preview page with components:', targetPage.components.length);
            ...
            console.log('PreviewPage: No target page found, initializing new project');
        ...
        console.error('PreviewPage: Error loading data:', error);
        // 出错时也初始化新项目
        const initialProject = getInitialProject((key: string) => key);
        setPreviewPage(initialProject.pages[0]);
        dispatch({ type: 'SET_PROJECT', payload: initialProject });
    ...
    loadData();
  }, [pageId, dispatch]); // 移除state.currentPage依赖，强制重新加载
  ...
  const handleRefreshPreview = () => {
    ...
    if (savedProject) {
      try {
        ...
        if (targetPage) {
          console.log('PreviewPage: Refreshing with latest data from localStorage:', targetPage);
          setPreviewPage(targetPage);
          // 同时更新编辑器状态
          dispatch({ type: 'SET_PROJECT', payload: project });
          dispatch({ type: 'SET_CURRENT_PAGE', payload: targetPage });
        ...
        console.error('PreviewPage: Error refreshing data:', error);
      }
    }
  };
