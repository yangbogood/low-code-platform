...
import { MailOutlined, PlusOutlined, EyeOutlined } from '@ant-design/icons';
...
export const EmailComponentLibrary: React.FC<EmailComponentLibraryProps> = ({
  ...
  const componentCategories = {
    '布局组件': emailComponents.filter(comp =>
      comp.type.includes('column') || comp.type === 'email-divider',
    ),
    '内容组件': emailComponents.filter(comp =>
      ['email-header', 'email-title', 'email-content', 'email-footer'].includes(comp.type),
    ),
    ...
    <div style={{ padding: '16px' }}>
      ...
      >
        <div style={{ fontSize: '12px', color: '#666' }}>
          <p><strong>1. 选择模板：</strong>点击预设模板快速开始</p>
          <p><strong>2. 拖拽组件：</strong>从组件库拖拽到画布</p>
          <p><strong>3. 编辑属性：</strong>选中组件后在右侧编辑属性</p>
          <p><strong>4. 预览邮件：</strong>点击预览查看邮件效果</p>
        </div>
      </Card>
    </div>
  );
